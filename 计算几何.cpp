...
struct polygon{
    ...
    //多边形和圆关系
    //2圆完全在多边形内,1圆在多边形里面，碰到了多边形边界,0其它
...
struct circles{
    circle c[maxn];
    double ans[maxp];//ans[i]表示被覆盖了i次的面积
    double pre[maxp];
    int n;
    circles(){}
    void add(circle cc){
    ...
    //测试SPOJVCIRCLESSPOJCIRUT
    //SPOJVCIRCLES求n个圆并的面积，需要加上init_or()去掉重复圆（否则WA）
    //SPOJCIRUT是求被覆盖k次的面积，不能加init_or()
